# JalaAgent Documentation

## Getting Started
- [Quickstart](quickstart.md) — 5-minute setup
- [User Guide](user-guide.md) — Manual book, how to use JalaAgent

## Core Concepts
- [Architecture](architecture.md) — System design and data flow
- [Memory System](memory.md) — 4-layer hybrid memory
- [Harness](harness.md) — Safe coding infrastructure
- [Configuration](configuration.md) — Full config.yaml reference

## Features
- [Skills Catalog](skills.md) — All 77 bundled skills
- [Commands Reference](commands.md) — 46 slash commands
- [Providers](providers.md) — 16+ provider setup

## Development
- [API Reference](api-reference.md) — Python API + REST API
- [Contributing Guide](contributing-guide.md) — How to contribute
- [Roadmap](roadmap.md) — What's next
